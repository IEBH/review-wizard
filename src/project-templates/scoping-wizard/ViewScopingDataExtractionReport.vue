...
export default {
	name: "ViewScopingDataExtractionReport",
	directives: {
	...
	computed: {
		table() {
			...
			concatedArray = concatedArray.concat(
				...
				this.$tera.state.concepts,
				this.$tera.state.contexts
			);
			//[Extraction] extractionThemes (if included)
			let filteredOutcome = this.$tera.state.extractionThemes.filter(
				item => item.main
			);
