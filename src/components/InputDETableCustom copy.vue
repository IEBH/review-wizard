	...
	<div class="p-mb-6">
		...
		<table class="p-fluid" style="width:100%;" v-if="value">
			<thead class="p-fluid-thead">
				<tr>
					<th v-for="(thead, index) in value" :key="index">
			...
			<tbody>
				...
				<tr v-for="(row, index) in value.rows" :key="index">
					<td v-for="thead of value" :key="thead.label">
						<Textarea :placeholder="thead.label" type="text" v-model="row[thead.label]" :ref="index"
							:autoResize="true" @change="changeHandler($event.target.value)" />
				...
				<tr>
					<td style="border-style: none;">
						<Button v-if="ifEdit" class="p-button-raised p-button-text" icon="pi pi-ellipsis-h"
							@click="show = !show" style="background-color:rgb(214, 116, 116)" />
					</td>
					<td style="border-style: none;">
	...
	watch: {
		value: {
			handler(newVal) {
				...
				console.log("iqqqq", newVal);

				if (newVal && !newVal.rows) {
					this.value['rows'] = [];
				}
				console.log("afteriqqqq", newVal);
			},
		},
	},
	methods: {
		...
		addRow(index, optionNum) {
			...
			for (var i = 0; i < this.value.length; i++) {
				if (this.value[i].label == "progress") {
					row[this.value[i].label] = {
						isComplete: "false",
						state: "Incomplete"
					};
				} else {
				...
				this.$set(row, row[this.value[i].label]);
			}
			if (optionNum == 0) {
		...
		addNewRow() {
			let data = { 'label': 'hellooo' };
			for (var i = 0; i < this.value.length; i++) {
				data[this.value[i].label] = "";
			}
			// this.value.push(data)
			// this.value.rows.push(data)
			console.log("placed", this.value)
			this.$emit("input", this.value);
			const newArrayToAdd = [10, 11, 12];

			// Using splice() method to add newArrayToAdd as the next element
			const indexToInsert = 1; // You can specify the index where you want to insert the new array
			this.value.splice(indexToInsert, 0, { name: 'New Object', data: newArrayToAdd });
